...
+ (void)initialize {
    ...
    NSMutableSet<YVSelector *> *demandableSelectors = [NSMutableSet new];
    // this captures in case we have superclass that has the demandables
    // we count on initialize happening from superclass down
    NSMutableSet<YVSelector *> *parentDemandableSelectors = objc_getAssociatedObject([self superclass], &kDemandableSelectorsKey);
    if (parentDemandableSelectors.count > 0) {
        [demandableSelectors addObjectsFromArray:parentDemandableSelectors.allObjects];
    }
    NSMutableSet<YVSelector *> *nodeableSelectors = [NSMutableSet new];
    NSMutableSet<YVSelector *> *parentNodeableSelectors = objc_getAssociatedObject([self superclass], &kNodeableSelectorsKey);
